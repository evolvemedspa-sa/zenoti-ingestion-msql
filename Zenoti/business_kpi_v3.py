...
import pandas as pd
import pyodbc
from db_helper import build_conn_str
...
def extract_date_from_filename(filename):
    # Daily format: Business_KPI_2025-01-15.csv -> 2025-01-15
    match = re.match(r'^Business_KPI_(\d{4}-\d{2}-\d{2})\.csv$', filename, re.IGNORECASE)
    if match:
        return match.group(1)
    # Monthly format: business_kpi_January-2025.csv -> first day of that month
    match = re.match(r'^business_kpi_([a-zA-Z]+)-(\d{4})\.csv$', filename, re.IGNORECASE)
    if match:
        month_num = MONTH_NUMBERS.get(match.group(1).lower())
        if month_num:
            return f"{match.group(2)}-{month_num:02d}-01"
    return None
    ...
    for f in sorted(os.listdir(CSV_FILE)):
        if f.lower().endswith('.csv'):
            if extract_date_from_filename(f):
                ...
                print(f"SKIPPED: '{f}' — filename must follow the format Business_KPI_<YYYY-MM-DD>.csv or business_kpi_<MonthName>-<YYYY>.csv")
elif os.path.isfile(CSV_FILE):
    fname = os.path.basename(CSV_FILE)
    if extract_date_from_filename(fname):
        csv_paths = [CSV_FILE]
    else:
        raise ValueError(f"Invalid filename: '{fname}' — filename must follow the format Business_KPI_<YYYY-MM-DD>.csv or business_kpi_<MonthName>-<YYYY>.csv")
else:
...
for csv_path in csv_paths:
    ...
    # ==================================
    # Upsert (replace-by-date): Remove any existing rows for this file's date,
    # then insert the fresh snapshot. A date with no prior rows simply inserts.
    # ==================================
    data_to_insert = df.astype(object).where(df.notnull(), None).values.tolist()

    try:
        if kpi_date and "business_kpi_date" in sql_column_lookup:
            date_col = sql_column_lookup["business_kpi_date"]
            delete_sql = f"DELETE FROM {table_qualified} WHERE [{date_col}] = ?"
            cursor.execute(delete_sql, (kpi_date,))
            print(f"Removed {cursor.rowcount:,} existing row(s) for business_kpi_date = {kpi_date}")
